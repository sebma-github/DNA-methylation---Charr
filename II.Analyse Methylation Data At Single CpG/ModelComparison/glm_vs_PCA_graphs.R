...
library(ggpubr)
#Load glm data
glmfinal <- read.csv("/Users/sebmatlosz/Desktop/methylation/glm_10340res_pvalues_corrected.csv")

# glmfinal <- read.csv("/Users/sebmatlosz/Desktop/methylation/glm_10340res_Niche_pvalues_corrected.csv")
#         glmfinal$Morph_pvalue <- glmfinal$Niche_pvalue
#         glmfinal$Morph_pvalue_corrected <- glmfinal$Niche_pvalue_corrected

#Load PCA data
PCAfinal <- read.csv("/Users/sebmatlosz/Desktop/methylation/weightsonPC_48samplesPCA.csv")
  
#Merge data frames based on residues with sqldf?
library(sqldf)

#Only keep the few PCs of interest / ATH Change Niche into Morphs or vice versa
...
#I need to start by checking if the data is parametric or not
qqnorm(df$Morph_pvalue_corrected_log) #Clearly not, because it is in log and stuff.
...
#Morphcorrnumber <- cor.test(df$PC4, df$Morph_pvalue_corrected_log, method = "pearson")
...
#Timecorrnumber <- cor.test(df$PC1, df$Timepoint_pvalue_corrected_log, method = "pearson")
...
pdf("/Users/sebmatlosz/Desktop/Figuresforpapers/ModelComparisons_Niche_minuslog_corrected_glmPCA_090222.pdf")
figure
dev.off()
